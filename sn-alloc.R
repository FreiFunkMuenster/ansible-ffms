...
download.file("https://graphite.freifunk-muensterland.de/render/?target=aliasSub(aliasByNode(scaleToSeconds(sumSeriesWithWildcards(gateways.*.interface-bat*.if_octets.*,1,4),1),1),%27interface-bat%27,%27domaene-%27)&format=json&from=NOW-7d&until=NOW-1h", "ffms-tx-rx.tmp.json")

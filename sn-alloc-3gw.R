...
download.file("https://graphite.freifunk-muensterland.de/render/?target=aliasByNode(aliasSub(sumSeriesWithWildcards(gateways.*.l2tp.if_count-br*,%201),%20%27if_count-br%27,%20%27domaene-%27),%202)&format=json&from=NOW-7d&until=NOW-1h", "ffms-l2tp.tmp.json")
download.file("https://graphite.freifunk-muensterland.de/render/?target=aliasSub(aliasByNode(scaleToSeconds(sumSeriesWithWildcards(gateways.*.interface-bat*.if_octets.*, 1, 4), 1), 1), 'interface-bat', 'domaene-')&format=json&from=NOW-7d&to=NOW-1h", "ffms-tx-rx.tmp.json")
...
if (file.exists("sn-alloc-manual.csv")) { 
	domains.manual <- read.table("sn-alloc-manual.csv", colClasses=c("character", "character", "character", "character"), sep="\t", header=TRUE)
	man.alloc <- TRUE
	message("Manual domain allocation file found:\n")
	print(domains.manual)
} else {
	domains.manual <- data.frame(dom=NA, gw1=NA, gw2=NA, gw3=NA)
	man.alloc <- FALSE
}

# Bestehende Allokationstabelle (zzgl. ggf. neuer Domains) auslesen, falls Datei vorhanden
if (file.exists("domains.csv")) { 
	domains.tab <- read.csv2("domains.csv", colClasses=c("character", "factor", "factor", "factor", "numeric", "numeric")) 
} else {
...
domains <- cbind(domains, gw1="", gw2="", gw3="")
levels(domains$gw1) <- levels(sn$name)
levels(domains$gw2) <- levels(sn$name)
levels(domains$gw3) <- levels(sn$name)

# Verteilung der Domänen

com <- data.frame(t(combn(sn$name, m=3)))
com <- rbind(com, data.frame(X1=com[,2],X2=com[,3],X3=com[,1]), data.frame(X1=com[,3],X2=com[,1],X3=com[,2]))

names(com) <- c("gw1","gw2","gw3")
com <- cbind(com, data.frame(srv1=sn$srv, srv2=sn$srv, srv3=sn$srv, l2tp=0, tx.rx=0, count=0))

for (i in row(com)[,1]) { 
	com$srv1[i] <- sn$srv[which(sn$name == com$gw1[i])]
	com$srv2[i] <- sn$srv[which(sn$name == com$gw2[i])]
	com$srv3[i] <- sn$srv[which(sn$name == com$gw3[i])]
}

# Alle Triplets entfernen, die auf dem gleich physischen Gerät liegen
com <- com[(com$srv1 != com$srv2) && (com$srv1 != com$srv3) && (com$srv2 != com$srv3) ,]
...
calc.perf <- function(srv,l2tp,tx.rx) { 
	...
	com[which(com$gw3 == srv),]$l2tp <<- com[which(com$gw3 == srv),]$l2tp + l2tp
	...
	com[which(com$gw3 == srv),]$tx.rx <<- com[which(com$gw3 == srv),]$tx.rx + tx.rx
}
...
if (man.alloc == TRUE) {
	for (i in row(domains.manual)[,1]) {
		...
		com.m <- (as.character(com$gw1) == domains.manual$gw1[i]) & (as.character(com$gw2) == domains.manual$gw2[i])  & (as.character(com$gw3) == domains.manual$gw3[i])
		domains$gw1[dom.m] <- domains.manual$gw1[i]
		domains$gw2[dom.m] <- domains.manual$gw2[i]
		domains$gw3[dom.m] <- domains.manual$gw3[i]
		...
		calc.perf(com$gw3[com.m], -num.l2tp, -num.tx.rx)
		...
		sn[sn$name == com$gw3[com.m],]$l2tp <- sn[sn$name == com$gw3[com.m],]$l2tp - num.l2tp
		...
		sn[sn$name == com$gw3[com.m],]$tx.rx <- sn[sn$name == com$gw3[com.m],]$tx.rx - num.tx.rx
	}
}
...
for (i in row(domains)[,1]) {
	if (!(domains$dom[i] %in% domains.manual$dom)) {
		...
		sn[sn$name == com$gw3[1],]$tx.rx <- sn[sn$name == com$gw3[1],]$tx.rx - num.tx.rx
	}
}
...
write.csv2(domains[c("dom","gw1","gw2", "gw3","l2tp","tx.rx")], file="domains.csv", row.names=FALSE)

# Domänenlisten in die host_vars-Dateien schreiben

list.domains <- function(dom) {
	domlist.new <- as.character()
	domlist <- as.character(domains$dom[c(which(domains$gw1 == dom), which(domains$gw2 == dom),  which(domains$gw3 == dom))])
...
for (i in sn$name) {
	...
	if (length(domlist) != 0) { 
		for (j in domlist) { 
			serverlist <-  c(sn[sn$name == domains[domains$dom == j,]$gw1,]$vm.id, sn[sn$name == domains[domains$dom == j,]$gw2,]$vm.id, sn[sn$name == domains[domains$dom == j,]$gw3,]$vm.id)	
			if (sn$vm.id[sn$name == i] == min(serverlist)) { dhcp.set <- calc.dhcp(j,offset=64,id=2) } 
